...
Before generating a new Fragment, Client Extension, Object definition, or Commerce product/SKU from scratch, **explicitly load the relevant skill from the Skill Router below**, even if no matching files exist yet in the workspace. Glob based autoloading will not fire on an empty workspace; this preflight step ensures the correct patterns and antihallucination guards are applied from the first line of code.
